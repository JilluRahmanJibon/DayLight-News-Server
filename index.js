...
async function run() {
  try {
    ...
    app.get('/entertainment', async (req, res) => {
      ...
      const entertainment = await allNewsCollection.find(query).limit(6).sort({ _id: -1 }).toArray()
      res.send(entertainment)

    })

    app.get('/lifeStyle', async (req, res) => {
      const query = { category: 'lifeStyle' }
      const lifeStyle = await allNewsCollection.find(query).limit(6).sort({ _id: -1 }).toArray()
      res.send(lifeStyle)

    })

    app.get('/cultures', async (req, res) => {
      const query = { category: 'cultures' }
      const cultures = await allNewsCollection.find(query).limit(6).sort({ _id: -1 }).toArray()
      res.send(cultures)
    ...
    app.get('/district', async (req, res) => {
      const query = { district: { $exists: true } }
      const result = await allNewsCollection.find(query).limit(50).toArray()
      res.send(result)
    })
    ...
    //social news
    app.post("/addSocialNews", async (req, res) => {
      const socialNews = req.body;
      const result = await SocialNewsCollection.insertOne(socialNews);
      res.send(result);
    });
